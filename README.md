...
首次使用链路自动切换时，还需在卡片中分别安装本机 Route Safety Helper 和 Mini 自愈组件；两个安装器各请求一次系统管理员授权，不保存密码。Route Safety Helper v5 安装后，普通出口切换、定点清理旧 Mini DNS 和符合条件的 Wi-Fi 自动 DNS 修复均使用精确 sudoers 命令，不再重复弹出授权框。
